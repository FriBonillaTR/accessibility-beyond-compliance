...
import { observable, attr, nullableNumberConverter } from '../../@microsoft/fast-element/dist/esm/index.js';
import { Direction, Orientation, keyHome, keyEnd, keyArrowDown, keyArrowLeft, keyArrowUp, keyArrowRight, limit } from '@microsoft/fast-web-utilities';
import { FormAssociatedSlider } from './slider.form-associated.js';
import { SliderModeEnum } from './slider.options.js';
...
class Slider extends FormAssociatedSlider {
    constructor() {
         ...
         * HTML Attribute: mode
         */
        this.mode = SliderModeEnum.singleValue;
         ...
         */
        this.handleWindowMouseUp = (_event) => {
            this.stopDragging();
        };
        this.stopDragging = () => {
            this.isDragging = false;
     ...
     * @internal
     */
    static convertPixelToPercent(pixelPos, minPosition, maxPosition, direction) {
    ...
    setThumbPositionForOrientation(direction) {
        const newPct = Slider.convertPixelToPercent(Number(this.value), Number(this.min), Number(this.max), direction);
    ...
    updateStepMultiplier() {
        const stepString = this.stepValue.toString();
    ...
    calculateNewValue(rawValue) {
        this.setupTrackConstraints();
        // update the value based on current position
        const newPosition = Slider.convertPixelToPercent(rawValue, this.orientation === Orientation.horizontal ? this.trackMinWidth : this.trackMinHeight, this.orientation === Orientation.horizontal ? this.trackWidth : this.trackHeight, this.direction);
...
], Slider.prototype, "instructionalText", void 0);
__decorate([
    attr
], Slider.prototype, "autocomplete", void 0);
__decorate([
    attr({ attribute: 'required', mode: 'boolean' })
], Slider.prototype, "required", void 0);
__decorate([
    attr({ attribute: 'required-text' })
], Slider.prototype, "requiredText", void 0);
